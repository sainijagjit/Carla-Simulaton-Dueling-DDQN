...
class World(object):
    ...
    def restart(self):
        ...
        #This is my image save camera
        cam_index_save=self.camera_manager.index if self.camera_manager is not None else 0
        cam_pos_index_save = self.camera_manager.transform_index if self.camera_manager is not None else 0
        ...
        self.camera_manager_save= CameraManager(self.player, self.hud, self._gamma)
        self.camera_manager_save.transform_index = cam_pos_index_save
        self.camera_manager_save.set_sensor(cam_index_save, notify=False)
    ...
    def render(self, display):
        self.camera_manager_save.render(display)
    ...
    def destroy_sensors(self):
        ...
        self.camera_manager_save.sensor.destroy()
        self.camera_manager_save.sensor = None
        self.camera_manager_save.index = None

    def destroy(self):
        if self.radar_sensor is not None:
            self.toggle_radar()
        actors = [
            self.camera_manager_save.sensor,
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.recording:
            #print('image is being saved now..!!')
            #image.save_to_disk('_out/%08d' % image.frame)
            self.saved_image=array
        #else:
            
            #print('image is not saving now...!!!')
...
DISCRETE_ACTIONS_SMALL = {
    ...
    9: [0.0, -0.70, 0.0, False, False],  # Left+Stop
    10: [0.0, -0.23, 0.0, False, False],  # Left+Stop
...
class env:
    ...
    def step(self,action):   
            ...
            self.world.ego2.apply_control(carla.VehicleControl(throttle=0.5,brake=0.0,steer=0.0))
    ...
    def get_state(self):
        ...
        cv2.imshow('Resized Window', self.world.camera_manager.saved_image)





        #cv2.imshow('Segmentated Image', self.world.camera_manager.saved_image)
        cv2.waitKey(1)
        #print(self.world.camera_manager.saved_image)
    ...
    def get_reward(self,indicator):
        ...
        def get_reward_new(self,indicator):
            ...
            #print(timediff)
            print('reward is {}'.format(posdiff))
            print('reward if time is a constraint {}'.format(round(posdiff/timediff,1)))
            print('reward is {}'.format(self.ego_x-self.start_pos))
            lane=False
...
if __name__ == '__main__':
    ...
    env = env() 
    no_of_iterations=2
    for ep in range(no_of_iterations):
